...
def build_dataset(bags, targets, encoder, bag_size, max_bag_size = None, use_lens=False):
    ...
    if encoder:
        dataset = MapDataset(
            _zip,
            BagDataset(bags, bag_size=bag_size, max_bag_size = max_bag_size),
...
def build_dataset_gcn(bags, edges, targets, encoder, bag_size, max_bag_size = None, use_lens=False):
    assert len(bags) == len(targets)
    assert len(edges) == len(bags)
    
    def _zip(bag, edge, targets):
        features, lengths = bag
        if use_lens:
            return (features, edge, lengths, targets.squeeze())
        else:
            return (features, edge, targets.squeeze())
    if encoder:
        dataset = MapDataset(
            _zip,
            BagDataset(bags, bag_size=None, max_bag_size = None),
            EncodedDataset(edges, IdentityEncoder()),
            EncodedDataset(encoder, targets),
        )
        dataset.encoder = encoder
    else:
...
class BagDataset(Dataset):
    ...
    def _load(self, index: int):
        ...
        elif isinstance(self.bags[index], np.ndarray) and not isinstance(self.bags[index][0], str):
    ...
    def __getitem__(self, index: int) -> Tuple[torch.Tensor, int]:
        ...
        if self.bag_size:
            ...
            if self.max_bag_size:
                if len(feats) > self.max_bag_size:
                    return _to_fixed_size_bag(feats, bag_size=self.max_bag_size)
