...
class TrainerConfigFastAI(_TrainerConfig):
    def __init__(
        ...
        **kwargs
        ...
        if model.lower() in ModelConfigFastAI.gcn_models:
            self.gcn = True
        else:
            self.gcn = False
        print("IS GCN?")
...
class ModelConfigFastAI(DictConfig):
    ...
    def model_fn(self):
        if not isinstance(self.model, str):
            ...
            return Transformer
        elif self.model.lower() == 'gcn':
            from slideflow.mil.models.gcn import GCN
            return GCN
        elif self.model.lower() == 'sequoia':
            from .models.vis import vis
            return vis
        elif self.model.lower() == 'deeppt':
            from .models.ml_reg import ml_reg
            return ml_reg
        else:
