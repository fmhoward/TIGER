...
def _eval_mil(
    ...
    **heatmap_kwargs
    ...
    if config.outcome_type == 'continuous':
        labels, _ = dataset.labels(outcomes, format='name', use_float=True)
    else:
    ...
    df, y_att, y_pred_tiles = predict_from_model(
        model,
        config,
        dataset,
        outcomes=outcomes,
    ...
    if outdir and y_att and attention_heatmaps:
        ...
        generate_attention_heatmaps(
            outdir=join(model_dir, 'heatmaps_pred'),
            dataset=dataset,
            bags=bags,  # type: ignore
            attention=[y_pred_tiles[:, 0]],
            **heatmap_kwargs
        )
        generate_attention_heatmaps(
            outdir=join(model_dir, 'heatmaps_pred1'),
            dataset=dataset,
            bags=bags,  # type: ignore
            attention=[y_pred_tiles[:, 1]],
            **heatmap_kwargs
        )
    return df
...
def predict_mil(
    ...
    **kwargs
    ...
    if config.outcome_type == 'continuous':
        labels, unique = dataset.labels(outcomes, format='name', use_float = True)
    else:
        labels, unique = dataset.labels(outcomes, format='id')
    ...
    if isinstance(bags, pd.DataFrame):
            patients = bags.patient.unique()
    else:
        patients = [slide_to_patient[path_to_name(b[0])] for b in bags]
    if config.outcome_type == 'continuous':
        df_dict = dict(patient=patients)
        df = pd.DataFrame(df_dict)
        df = pd.concat((df, pd.DataFrame(y_true)), axis = 1)
    elif config.outcome_type == 'treatment':

        df_dict = dict(patient=patients, y_true=y_true)
        for i in range(treatment.shape[-1]):
            df_dict[f'treatment_{i}'] = treatment[:, i]
    else:
        df_dict = dict(patient=patients, y_true=y_true)
    ...
    if config.outcome_type == "continuous":
        ...
        df.columns = ['slide'] + ['ytrue' + str(i) for i in range(y_true.shape[-1])] + ['ypred' + str(i) for i in range(y_pred.shape[-1])]
    else:
        for i in range(y_pred.shape[-1]):
            df_dict[f'y_pred{i}'] = y_pred[:, i]
        if uq:
...
def run_inference(
    ...
    *,
    ...
    if uq and inspect.signature(model.forward).parameters['uq']:
        ...
        kw = dict()
    if attention and 'return_attention' in inspect.signature(model.forward).parameters and inspect.signature(model.forward).parameters['return_attention']:
...
def _to_fixed_size_bag(
    ...
    zero_padded = torch.cat(
        (
            bag_samples,
            torch.zeros(bag_size - bag_samples.shape[0], bag_samples.shape[1]).to(device),
        )
    )
