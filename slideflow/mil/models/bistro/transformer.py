...
def _compute_rollout(all_layer_matrices, start_layer=0):
    with torch.cuda.amp.autocast():
        ...
        matrices_aug = [matrices_aug[i].cpu() for i in range(len(matrices_aug))]
        joint_attention = matrices_aug[start_layer].cpu()
            ...
            if matrices_aug[i].shape[2] > 5000:
                ...
                if i == len(matrices_aug) - 1:
                    for chunk in torch.split(matrices_aug[i], 5000, dim = 1):
                        results.append(chunk.bmm(joint_attention)[:,0, 1:])
                else:
                    for chunk in torch.split(matrices_aug[i], 5000, dim = 1):
                        results.append(chunk.bmm(joint_attention))
                joint_attention = torch.cat(results, dim = 1)
             
            else:
                joint_attention = matrices_aug[i].bmm(joint_attention)
                if i == len(matrices_aug) - 1:
                    joint_attention = joint_attention[:,0, 1:]
        return joint_attention.to('cuda:0')
...
class Transformer(nn.Module):
    ...
    def _calculate_rollout_attention(self, start_layer=0):
        ...
        for blk in blocks:
            ...
            avg_heads = attn_heads.detach()#(attn_heads.sum(dim=1) / attn_heads.shape[1]).detach()
