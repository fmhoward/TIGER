    ...
    *,
    ...
    for patient, patient_bags in patient_to_bags.items():
        ...
        if len(np.unique([labels[path_to_name(b)] for b in patient_bags], axis = 0)) != 1:
            print(patient)
            print(patient_bags)
            print([labels[path_to_name(b)] for b in patient_bags])
            raise ValueError(
                "Patient {} has slides/bags with different labels".format(patient))
        patients_labels[patient] = labels[path_to_name(patient_bags[0])]
...
def aggregate_trainval_bags_by_patient_gcn(
    bags: np.ndarray,
    edgelist: np.ndarray,
    labels: Dict[str, int],
    train_slides: List[str],
    ...
    *,
    ...
    patient_to_bags = {}  # type: Dict[str, List[str]]
    patient_to_edges = {}
    for bag, edge in zip(bags, edgelist):
        patient = slide_to_patient[path_to_name(bag)]
        if patient not in patient_to_bags:
            patient_to_bags[patient] = []
            patient_to_edges[patient] = np.array([])   
        patient_to_bags[patient].append(bag)
        edgelist = patient_to_edges[patient]
        num_edges = len(edge_list)
        for i in len(edge):
            for j in len(edge[i]):
                edge[i][j] = edge[i][j] + num_edges
        patient_to_edges[patient] = np.append(patient_to_edges[patient], edge)

    # Create array where each element contains the list of slides for a patient.
    bags = np.array([lst for lst in patient_to_bags.values()], dtype=object)
    edges = np.array([lst for lst in patient_to_edges.values()], dtype=object)
    ...
    return bags, edges, targets, train_idx, val_idx
