    ...
    if exp_label is None:
        try:
            exp_label = '{}-{}'.format(
                ...
                "-".join(outcomes if isinstance(outcomes, list) else [outcomes])[:50]
            )
        except Exception:
            exp_label = 'no_label'
...
def build_fastai_learner(
    ...
    *,
    ...
    if config.outcome_type == "categorical":
        ...
        labels, unique_train = train_dataset.labels(outcomes, format='name', use_float = True)
        val_labels, unique_val = val_dataset.labels(outcomes, format='name', use_float = True)
        labels.update(val_labels)
...
def build_fastai_learner_gcn(
    ...
    *,
    ...
    if r is not None and k is not None:
        raise ValueError('Only one of r or k should be specified.')
    if r is None and k is None:
        raise ValueError('Either r or k should be specified.')
    
    train_edges = []
    for slide, bag in zip(train_slides, train_bags): 
        coordinates = torch.from_numpy(np.load(bag[:-3] + ".index.npz")['arr_0']).to(torch.float32)
        if r is not None:
            edge_index = torch_geometric.nn.radius_graph(coordinates, r=r)
        else:
            edge_index = torch_geometric.nn.knn_graph(coordinates, k=k)
        train_edges += [edge_index]
        #data = Data(x=features, pos=coordinates, edge_index=edge_index,y=labels)#, y=torch.tensor(labels, dtype=torch.float32))
        #data.slide = slide
        #if(outcome_type=="continuous"):
        #    labels_array = np.array(labels)
        #    labels_numeric = [float(value) for value in labels]
        #    labels_array = np.array(labels_numeric)
        
        #    data.targets = torch.tensor(labels_array, dtype=torch.float32)
        #if subsample is not None:
        #    data = subsample_graph(data, subsample)
        #return data
    val_edges = []
    for slide, bag in zip(val_slides, val_bags): 
        coordinates = torch.from_numpy(np.load(bag[:-3] + ".index.npz")['arr_0']).to(torch.float32)
        if r is not None:
            edge_index = torch_geometric.nn.radius_graph(coordinates, r=r)
        else:
            edge_index = torch_geometric.nn.knn_graph(coordinates, k=k)
        val_edges += [edge_index]        
    
    edges = np.concatenate((train_edges, val_edges))
    ...
    if config.aggregation_level == 'slide':
        ...
        # Aggregate feature bags across patients.
        n_slide_bags = len(bags)
        bags, targets, train_idx, val_idx = utils.aggregate_trainval_bags_by_patient_gcn(
            bags,  # type: ignore
            edges,
            labels,
            train_slides,
...
def train_fastai(
    ...
    **heatmap_kwargs
    ...
    if config.gcn:
        print("Building GCN Learner")
        # Build learner.
        learner, (n_in, n_out) = build_fastai_learner_gcn(
            config,
            train_dataset,
            val_dataset,
            outcomes,
