...
def train(learner, config, callbacks=None):
    ...
    """
    cbs = [
        SaveModelCallback(fname=f"best_valid", monitor=config.save_monitor, every_epoch = config.every_epoch),
        CSVLogger(),
    ]
    if callbacks:
        cbs += callbacks
...
def build_learner(config, *args, **kwargs) -> Tuple[Learner, Tuple[int, int]]:
    ...
    if isinstance(config.model_config, ModelConfigCLAM):
        ...
        if config.gcn:
            return _build_fastai_learner_gcn(config, *args, **kwargs)
        else:
...
def _build_fastai_learner_gcn(
    ...
    **dl_kwargs
    ...
    val_dataset = data_utils.build_dataset_gcn(
        bags[val_idx],
        edges[val_idx],
        targets[val_idx],
        encoder=encoder,
